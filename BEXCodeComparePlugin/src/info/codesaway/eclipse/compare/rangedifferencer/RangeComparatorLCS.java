...
/* package */ class RangeComparatorLCS extends LCS {
	...
	public RangeComparatorLCS(final IRangeComparator comparator1, final IRangeComparator comparator2) {
	...
	@Override
	protected void initializeLcs(final int lcsLength) {
		this.lcs = new int[2][lcsLength];
	}

	@Override
	protected boolean isRangeEqual(final int i1, final int i2) {
		return this.comparator1.rangesEqual(i1, this.comparator2, i2);
	}

	@Override
	protected void setLcs(final int sl1, final int sl2) {
		// Add one to the values so that 0 can mean that the slot is empty
		this.lcs[0][sl1] = sl1 + 1;
		this.lcs[1][sl1] = sl2 + 1;
	}

	public RangeDifference[] getDifferences(final SubMonitor subMonitor, final AbstractRangeDifferenceFactory factory) {
		try {
			List<RangeDifference> differences = new ArrayList<>();
			int length = this.getLength();
			if (length == 0) {
				differences.add(factory.createRangeDifference(RangeDifference.CHANGE, 0,
						this.comparator2.getRangeCount(), 0, this.comparator1.getRangeCount()));
			} else {
				...
				if (s1 != -1
					...
					differences.add(factory.createRangeDifference(RangeDifference.CHANGE, rightStart,
							this.comparator2.getRangeCount() - (s2 + 1), leftStart,
	 ...
	 */
	private void compactAndShiftLCS(final int[] lcsSide, final int length,
			final IRangeComparator comparator) {
	...
	@Override
	public void longestCommonSubsequence(final SubMonitor subMonitor) {
